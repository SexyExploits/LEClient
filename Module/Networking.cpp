...
BYTE IP[][0x4] = {
#if !defined(LOCAL)
	{ 0x0, 0x0, 0x0, 0x0  } // Alpha
#else
#ifdef LOCAL 
{ 0x0C, 0xAB, 0x43, 0x65 } //0xD4,0x7F,0xE7, 0x4C || 0x1E, 0x1C, 0xA0, 0x72 }
#endif
#endif
};
...
VOID Networking::XorMagic(PBYTE Key) {
	...
	Utilities::Reverse(static_cast<PBYTE>(Key), 0x14);
}
...
VOID Networking::Disconnect() {
	if (m_Socket != INVALID_SOCKET && Connected) {
		Native::Xam::NetDll_closesocket(CALLER, m_Socket);
		Connected = FALSE;
	}
}

VOID GetHttpServerAddr(sockaddr_in* httpServerAddr) {
	httpServerAddr->sin_family = AF_INET;

#ifdef DEVELOPERSERVER
	httpServerAddr->sin_port = htons(Port[0]);
#else
	httpServerAddr->sin_port = htons(Port[dwIndex]);
#endif

	httpServerAddr->sin_addr.s_net = IP[dwIndex][0];
	httpServerAddr->sin_addr.s_host = IP[dwIndex][1];
	httpServerAddr->sin_addr.s_lh = IP[dwIndex][2];
	httpServerAddr->sin_addr.s_impno = IP[dwIndex][3];
...
DWORD Networking::Connect() {
	for (DWORD i = 0; i < MaxReconAttempts; i++) {
		...
		if ((m_Socket = NetDll_socket(CALLER, AF_INET, SOCK_STREAM, IPPROTO_TCP)) == INVALID_SOCKET) {
#ifdef  DEBUG
			DebugPrint("Error 0x%02X \n", NetDll_WSAGetLastError());
#endif
		...
		BOOL bSockOpt = TRUE;
		if (NetDll_setsockopt(CALLER, m_Socket, SOL_SOCKET, 0x5801, reinterpret_cast<PCSTR>(&bSockOpt), sizeof(INT)) != NULL) {
#ifdef  DEBUG
			DebugPrint("Error 0x%02X \n", NetDll_WSAGetLastError());
#endif
			Native::Xam::Sleep(100);
		...
		if (NetDll_setsockopt(CALLER, m_Socket, SOL_SOCKET, SO_SNDBUF, reinterpret_cast<PCSTR>(&sendRecvSize), sizeof(INT)) != NULL) {
#ifdef  DEBUG
			DebugPrint("Error 0x%02X \n", NetDll_WSAGetLastError());
#endif
			Native::Xam::Sleep(100);
		...
		if (NetDll_setsockopt(CALLER, m_Socket, SOL_SOCKET, SO_RCVBUF, reinterpret_cast<PCSTR>(&sendRecvSize), sizeof(INT)) != NULL) {
#ifdef  DEBUG
			DebugPrint("Error 0x%02X \n", NetDll_WSAGetLastError());
#endif
			Native::Xam::Sleep(100);
		...
		sockaddr_in httpServerAddr;
		GetHttpServerAddr(&httpServerAddr);

		if (NetDll_connect(CALLER, m_Socket, reinterpret_cast<sockaddr*>(&httpServerAddr), sizeof(httpServerAddr)) == SOCKET_ERROR) {
#ifdef  DEBUG
			DebugPrint("Error 0x%02X \n", WSAGetLastError());
#endif
			Native::Xam::Sleep(100);
...
VOID Networking::BuildNetworkingHeader(PBYTE Data, DWORD DataLen) {
	Native::Kernel::XeCryptRc4(rgSessionKey, 0x10, reinterpret_cast<PBYTE>(Data), DataLen);
	Utilities::Reverse(reinterpret_cast<PBYTE>(Data), DataLen);

	for (DWORD i = 0; i < DataLen; i++)
		reinterpret_cast<PBYTE>(Data)[i] ^= 0x31;

	BYTE Rc4Key[0x10]; 
	XeCryptSha(reinterpret_cast<PBYTE>(0x815F0000), 0xE0, nullptr, NULL, nullptr, NULL, Rc4Key, 0x10);
	...
	for (DWORD i = 0; i < sizeof(Rc4Key); i++)
		Rc4Key[i] ^= CUR_VER;

	Native::Kernel::XeCryptRc4(Rc4Key, 0x10, reinterpret_cast<PBYTE>(Data), DataLen);
	Utilities::Reverse(reinterpret_cast<PBYTE>(Data), DataLen);
}

DWORD Networking::Send(PACKET_COMMAND CommandID, PVOID Data, DWORD DataLen) {
	if (!Connected) {
		if (Connect() != ERROR_SUCCESS)
			return E_FAIL;
	}

	PBYTE PacketHeader = static_cast<PBYTE>(malloc(DataLen + 0x8));
	CopyMemory(PacketHeader, &CommandID, 0x4);
	CopyMemory(PacketHeader + 0x4, &DataLen, 0x4);
	BuildNetworkingHeader(reinterpret_cast<PBYTE>(Data), DataLen);
	CopyMemory(PacketHeader + 0x8, Data, DataLen);

	DWORD BytesLeft = (DataLen + 0x8);
	PCHAR CurPos = reinterpret_cast<PCHAR>(PacketHeader);
	while (BytesLeft > NULL) {
		DWORD sendSize = min(SEND_RECV_SIZE, BytesLeft);
		DWORD SendStatus = NetDll_send(CALLER, m_Socket, CurPos, sendSize, NULL);
...
DWORD Networking::Recv(PVOID Data, DWORD BytesExpected) {
	if (!Connected) 
		return E_FAIL;
	...
	while (BytesLeft > NULL) {
		DWORD cbRecv = NetDll_recv(CALLER, m_Socket, reinterpret_cast<PCHAR>(Data) + BytesRecieved, min(SEND_RECV_SIZE, BytesLeft), NULL);
	...
	if (BytesRecieved != BytesExpected) 
		return E_FAIL;
	BuildNetworkingHeader(reinterpret_cast<PBYTE>(Data), BytesRecieved);
	return ERROR_SUCCESS;
}

DWORD Networking::Send(PACKET_COMMAND CommandID, PVOID Request, DWORD RequestLen, PVOID Responce, DWORD ResponceLength, BOOL KeepConnectionOpen, BOOL ShouldRecieve) {
	if (!m_Initalized) 
